...
%bcond_without xcpng
%if %{with xcpng}
%bcond_with python # Disable feature, not used before and can bring security issue.
%bcond_with selinux # Disable feature, XCP-ng do not support it.
%endif
...
Requires: pam
# XCP-ng, XenServer had put this Requires, it is probably still
# necessary because we depend exclusively on vi for this package.
Requires: /usr/bin/vi
...
# XCP-ng does not use selinux. So this is not needed.
# XCP-ng does not have systemd-rpm-macros.
# This is only useful for %{_tmpfilesdir}
# But systemd already provide it (/usr/lib/tmpfiles.d)
%if %{with selinux}
...
# XCP-ng never supported logsrvd
Obsolete: logsrvd <= 1.9.15-5.1

# This package can bring security issue.
# We didn't have it before, so we will disable it.
%if %{with python}
...
# XCP-ng: Disable SELinux, because we did not use it.
# XCP-ng: Disable python, because we did not provide sudo-python-plugin.
%configure \
        --prefix=%{_prefix} \
        --sbindir=%{_sbindir} \
        --libdir=%{_libdir} \
...
# sesh is only built when SELinux support is enabled.
%if %{with selinux}
...
%changelog
* Tue Sep 15 2026 Lucas RAVAGNIER <lucas.ravagnier@vates.tech> - 1.9.17-10.p2.1
...
* Fri Jun 12 2026 Alejandro López <allopez@redhat.com> - 1.9.17-9.p2
- Removed some unneeded build-time dependencies

* Fri Jun 12 2026 Alejandro López <allopez@redhat.com> - 1.9.17-8.p2
- Resolves: RHEL-95850 - A sudo'd command when invoked from ksh through a
  pipe mangles output, leaves tty insane

* Fri Jun 12 2026 Alejandro López <allopez@redhat.com> - 1.9.17-7.p2
- Use @includedir instead of #includedir in sudoers

* Thu Jun 04 2026 Alejandro López <allopez@redhat.com> - 1.9.17-6.p2
- Resolves: RHEL-1354 - Outdated example entry in /etc/sudoers, need to
  update /etc/sudoers file

* Thu Jun 04 2026 Alejandro López <allopez@redhat.com> - 1.9.17-5.p2
- Resolves: RHEL-139077 - sudo rpm(s) have weak dependencies in BaseOS
