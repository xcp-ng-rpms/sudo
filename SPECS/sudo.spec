...
# Previous versions of our sudo package didn't provide these.
# No need to support nor build them.
%if %{with xcpng}
%bcond_with selinux
%bcond_with python-plugin
%endif
...
# XCP-ng's 8.3's rpm doesn't support Recommends tags,
# doesn't provide a system-default-editor package.
%if %{without xcpng}
Recommends: system-default-editor
%if %{with python-plugin}
...
# SELinux support historically disabled in XenServer's sudo package and not required for XCP-ng features
%if %{with selinux}
BuildRequires: libselinux-devel
%endif
# We don't have systemd-rpm-macros
%if %{without xcpng}
...
# Disable intercept to enable script using sudo (Compatibility)
# Disable SELinux (Compatibility)
...
# Disabling SELinux need to unpackage this
%if %{with selinux}
...
- Drop logsrvd for security reason
